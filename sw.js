...
"const CACHE_NAME = 'turbotartaruga-202604172116';
  ...
  for (const n of pending) {
    if (n.at <= now) {
      ...
      await self.registration.showNotification(n.title, {
        ...
        icon: './icon-192.png',
        badge: './icon-192.png',
        tag: n.tag || 'turbotartaruga-202604172116',
        renotify: true,
...
self.addEventListener('message', async event => {
  ...
  if (event.data.type === 'CANCEL_TODAY_NOTIF') {
    ...
    const pending = await loadPendingNotifs();
    const filtered = pending.filter(n => n.tag !== 'turbotartaruga-202604172116');
    await savePendingNotifs(filtered);
    // Close any shown notification with that tag
    self.registration.getNotifications({ tag: 'turbotartaruga-202604172116' })
  ...
  if (event.data.type === 'SCHEDULE_NOTIF') {
    ...
    const notifTag = tag || 'turbotartaruga-202604172116';
